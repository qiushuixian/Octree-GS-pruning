    ...
    for scene_dir in model_paths:
        ...
        for method in os.listdir(test_dir):
            ...
            for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                ...
                print(f'{idx}:{l1_loss(renders[idx], gts[idx]).double()}')
                
                loss+=l1_loss(renders[idx], gts[idx]).double()
            print("  SSIM : {:>12.7f}".format(torch.tensor(ssims).mean(), ".5"))
            print("  PSNR : {:>12.7f}".format(torch.tensor(psnrs).mean(), ".5"))
            print("  LPIPS: {:>12.7f}".format(torch.tensor(lpipss).mean(), ".5"))
            print("  GS   : {:>12.7f}".format(torch.tensor(gss).float().mean(), ".5"))
            print(f'total_num:{len(renders)}')
            print(f'loss_average:{loss/len(renders)}')
