...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    # learnable_para
    learnable_para = torch.tensor([9.0, 9.0, 9.0, 9.0, 9.0, 9.0, 9.0, 9.0, 9.0, 9.0], requires_grad=True, device="cuda")
    confignew.learnable_para_mid = learnable_para
    #print(f"confignew.learnable_para_mid: {confignew.learnable_para_mid}")  # 添加调试信息
    learnable_para_optimizer = torch.optim.SGD([learnable_para], lr=0.1)  # 定义优化器
    
    for iteration in range(first_iter, opt.iterations + 1):
        config.iteration_para = iteration
        ...
        confignew.viewmatrix_new = viewpoint_cam.world_view_transform
        confignew.fx_mid=viewpoint_cam.FoVx
        confignew.fy_mid=viewpoint_cam.FoVy
        #print(viewpoint_cam.world_view_transform)
        #scene.gaussians.plot_levels()
        confignew.flag_mask = 1
        # 修改：在设置 anchor mask 时使用 learnable_para
        #gaussians.plot_levels()
        gaussians.set_anchor_mask(viewpoint_cam.camera_center, iteration, viewpoint_cam.resolution_scale)
        confignew.flag_mask = 0
        ...
        image, viewspace_point_tensor, visibility_filter, offset_selection_mask, radii, scaling, opacity = render_pkg[
            "render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["selection_mask"], \
                                                              render_pkg["radii"], render_pkg["scaling"], render_pkg[
                                                                  "neural_opacity"]
        ...
        # 修改：添加与 learnable_para 相关的正则化项
        para_reg = learnable_para.norm() * 0.0001  # 可根据实际情况调整权重
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + 0.01 * scaling_reg

        confignew.loss_of_para = loss
        loss.backward()

        iter_end.record()
        confignew.learnable_para_mid = learnable_para
        #print(learnable_para)
        with torch.no_grad():
            # Progress bar
            ...
            training_report(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                            testing_iterations, scene, render, (pipe, background), wandb, logger)
            ...
            if iteration < opt.iterations:
                ...
                learnable_para_optimizer.step()  # 更新 learnable_para
                learnable_para_optimizer.zero_grad()  # 清零梯度
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    confignew.viewmatrix_new=viewpoint.world_view_transform
                    #结束修改
                    #print(type(viewpoint.world_view_transform))
                    #print(type(viewpoint))
                    #print(type(confignew.viewmatrix_new))
                    #scene.gaussians.plot_levels()
                    confignew.flag_mask=1
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        confignew.viewmatrix_new=view.world_view_transform
        #结束修改
        print(type(view.world_view_transform))
        print(type(view))
        print(type(confignew.viewmatrix_new))
        #gaussians.plot_levels()
        confignew.flag_mask=1
