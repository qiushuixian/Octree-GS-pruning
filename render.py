...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, show_level, ape_code):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        config.viewmatrix_new=view.world_view_transform
        #print(view.world_view_transform)
        confignew.fx_mid=view.FoVx
        confignew.fy_mid=view.FoVy
        ...
        config.p0=1
    #print(f"render执行时间: {total_time:.4f} 秒")
    t = np.array(t_list[5:])
    fps = 1.0 / t.mean()
    
    print(f'Test FPS: \033[1;35m{fps:.5f}\033[0m')
    print(f'rate;{(confignew.before_mask_sum-confignew.after_mask_sum)/x}')
    print(f'after_mask_sum;{confignew.after_mask_sum/x}')
    print(f'before_mask_sum;{confignew.before_mask_sum/x}')
